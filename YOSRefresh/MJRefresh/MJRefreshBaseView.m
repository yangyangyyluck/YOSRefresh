...
/**
 *  使用自己绘制的动画view 下面的arrowImage弃用
 */
- (TPullRefreshView *)pullRefreshView {
    if (!_pullRefreshView) {
        TPullRefreshView *pullRefreshView = [TPullRefreshView pullRefreshView];
...
//        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcName(@"arrow.png")]];
        ...
        NSString *urlStr = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"MJRefresh.bundle/arrow@2x.gif"];
        
        UIImage *image = [[UIImage alloc] initWithContentsOfFile:urlStr];
//        UIImage *image = [UIImage yos_animatedImageWithAnimatedGIFData:[NSData dataWithContentsOfFile:urlStr] withDivide:60];
        
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:image];
        
        arrowImage.frame = CGRectMake(0, 0, 60, 60);
...
//    self.arrowImage.center = CGPointMake(arrowX, self.mj_height * 0.5);
    ...
    // 自动以动画
    self.pullRefreshView.center = CGPointMake(self.mj_width * 0.5, self.mj_height * 0.5 - 10);
