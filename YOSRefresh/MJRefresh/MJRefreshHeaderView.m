...
//    self.statusLabel.frame = self.bounds;
    ...
    self.statusLabel.frame = CGRectMake(0, 40, self.mj_width, self.mj_height - 40);
...
- (void)updateTimeLabel
{
    if (!self.lastUpdateTime) {
        self.lastUpdateTimeLabel.text = @"首次更新";
        return;
    }
    // 1.获得年月日
...
- (void)adjustStateWithContentOffset
{
    ...
    CGFloat yOffset = currentOffsetY - happenOffsetY;
    
    if (yOffset < 0) {
        yOffset = ABS(yOffset);
        
        CGFloat percentage = yOffset / MJRefreshViewHeight;
        percentage = MIN(1.0, percentage);
        
        self.pullRefreshView.percentage = percentage;
    }
    ...
    if (self.scrollView.isDragging) {
        ...
        [self.pullRefreshView executeCustomAnimation];
    }
}
